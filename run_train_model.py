...
parser.add_argument('--name', default="649", type=str, help="选择训练数据: 双色球/大乐透")
...
def train_with_eval_all_ball_model(name, x_train, y_train, x_test, y_test):
    """ 红球模型训练与评估 """
    m_args = model_args[name]
    x_train = x_train - 1
    y_train = y_train - 1
...
def run(name, train_test_split):
    ...
    if name == "ssq" or name == "dlt":
        ...
        logger.info("开始训练【{}】红球模型...".format(name_path[name]["name"]))
        train_with_eval_all_ball_model(
            name,
            x_train=train_data["red"]["x_data"], y_train=train_data["red"]["y_data"],
            x_test=test_data["red"]["x_data"], y_test=test_data["red"]["y_data"],
        )
